...
if [ "$1" == "media" ]; then
    ...
    OP_PRELOAD_COPY_DONE="/data/media/copy_op_preload_done.ini"
    OP_PRELOAD_DONE="/data/system/op_preload_done.ini"
    PRELOAD_LINK_LOCATION_DIR="/data/media/0/Preload"

    if [ ! -f ${OP_PRELOAD_DONE} ] && [ ! -f ${OP_PRELOAD_COPY_DONE} ]; then
            ...
            if [ -d ${OP_PRELOAD_DIR} ]; then
                ...
                for PRELOAD_ITEM in ${PRELOAD_LIST}; do
                    if [ -f ${OP_PRELOAD_DIR}/${PRELOAD_ITEM} ]; then
                        if [ -L ${PRELOAD_LINK_LOCATION_DIR}/${PRELOAD_ITEM} ]; then
                            rm -rf ${PRELOAD_LINK_LOCATION_DIR}/${PRELOAD_ITEM}
                        fi
                        cp -rf ${OP_PRELOAD_DIR}/${PRELOAD_ITEM} ${PRELOAD_LINK_LOCATION_DIR}/${PRELOAD_ITEM}
                    fi
                done
                chown -R media_rw:media_rw ${PRELOAD_LINK_LOCATION_DIR}
                chmod -R 0775 ${PRELOAD_LINK_LOCATION_DIR}
            fi
            echo "op_preload_done" > ${OP_PRELOAD_COPY_DONE}
        fi
    fi
    exit 0
fi
