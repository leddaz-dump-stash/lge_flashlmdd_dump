...
if [[ $file_cnt > 0 ]]; then
    if [[ $log_to_data_partition == 1 ]]; then
        ...
        /system_ext/bin/kernel_logger -f /data/logger/${log_file} -s $file_size -m $file_cnt
    else
        ...
        /system_ext/bin/kernel_logger -f /${tmp_log_path}/encryption_log/${log_file} -s $file_size -m $file_cnt
    fi
else
