...
cp /product/etc/appmanager.conf ${TARGET_ETC_PATH}/

# Read NT-Code MCC
NTCODE=$(laop_cmd getprop ro.vendor.lge.ntcode_mcc XXX)
...
MCCMNC_LIST=$(laop_cmd getprop persist.vendor.lge.mccmnc-list "FFFFF")
...
if  [ "$EEA_PATH" = "4c" ] ; then
    ...
    if [ "$IS_OVERLAY_FS" != "true" ] ; then
        cp -rpf ${SOURCE_ETC_PATH}/sysconfig/non-eea_v1_search_chrome.xml ${TARGET_ETC_PATH}/eea_v1_search_chrome.xml
        log -p i -t "${LOG_TAG}" "[SBP] cp -rpf ${SOURCE_ETC_PATH}/sysconfig/non-eea_v1_search_chrome.xml ${TARGET_ETC_PATH}/eea_v1_search_chrome.xml"
    fi
fi

BELL_FACEBOOK_APPMANAGER=$(getprop persist.facebook.partnerid)
if [ ! -z "${BELL_FACEBOOK_APPMANAGER}" ]; then
    log -p e -t "${LOG_TAG}" "[SBP] use FACEBOOK Partner ID for BELL"
    echo ${BELL_FACEBOOK_APPMANAGER} > ${TARGET_ETC_PATH}/appmanager.conf
fi
