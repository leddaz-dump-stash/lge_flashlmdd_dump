...
if [ $# -eq 1 ]; then
  ...
  SYSTEM_EXT_PRELOAD_DIR="/system_ext/preload"
  PRODUCT_PRELOAD_DIR="/product/preload"
  DATA_PRELOAD_DIR="/data/preload"
  PRODUCT_MEDIA_DIR="/product/media/music"
  DATA_LG_MEDIA_DIR="/data/media/0/Preload/LG"
  MEDIA_PRELOAD_DIR="/data/media/0/Preload"

  if [[ "${mountpoint}" == "${SYSTEM_EXT_PRELOAD_DIR}" ]] ; then
    do_copy_folder ${SYSTEM_EXT_PRELOAD_DIR}/ ${DATA_PRELOAD_DIR}/
  elif [[ "${mountpoint}" == "${PRODUCT_PRELOAD_DIR}" ]] ; then
    do_copy_folder ${PRODUCT_PRELOAD_DIR}/ ${DATA_PRELOAD_DIR}/
  elif [[ "${mountpoint}" == "${PRODUCT_MEDIA_DIR}" ]] ; then
    if [ ! -d "${MEDIA_PRELOAD_DIR}" ]; then
      mkdir -p ${MEDIA_PRELOAD_DIR}
    fi
    chmod 775 ${MEDIA_PRELOAD_DIR}
